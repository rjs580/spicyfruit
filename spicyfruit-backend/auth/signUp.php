<?php
  ...
  if(json_last_error() === JSON_ERROR_NONE && isset($postdata->userType)) {
    if(($postdata->userType === SocialUser || $postdata->userType === Regular) && isset($postdata->userEmail) && isset($postdata->nickName)) {
      $userType = $postdata->userType;
      $email = $postdata->userEmail;
      $nickName = urlencode($postdata->nickName);
  ...
  function verifyPassword($password) {
    ...
    return ($letter && $number && strlen($password) >= 8);
  }